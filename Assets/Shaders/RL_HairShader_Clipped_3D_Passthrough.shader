// ★GuideChuna 사본 — 원본: cc_unity_tools_3D-main/3D/Shaders/Amplify/RL_Amplify_HairShader_Clipped_3D.shader
// 원본을 고치면 외부 에셋 업데이트 때 날아가므로 사본을 둔다. 원본과의 차이는 둘뿐이다:
//   1) Shader 이름   2) c.a = alpha518 -> c.a = 1  (패스스루 누출 차단, 2026-08-27)
// 이 셰이더를 쓰는 머티리얼: c9/Hair_Transparency.mat, c9/Scalp_Transparency.mat
// Made with Amplify Shader Editor v1.9.0.1
// Available at the Unity Asset Store - http://u3d.as/y3X 
Shader "GuideChuna/RL_HairShader_Clipped_3D_Passthrough"
{
	Properties
	{
	...
	SubShader
	{
		...
		inline half4 LightingStandardCustomLighting( inout SurfaceOutputCustomLightingCustom s, half3 viewDir, UnityGI gi )
		{
			...
			// ★GuideChuna 개변: 원본은 alpha518을 그대로 프레임버퍼에 쓴다(keepalpha).
			//   패스스루는 프레임버퍼 알파로 합성하므로 현실 배경·사용자 손이 머리카락을 통해 비쳤다.
			//   바로 위 clip()이 투명 픽셀을 이미 버렸으므로, 여기까지 온 픽셀은 알파 1이 맞다.
			c.a = 1;
			return c;
		}
