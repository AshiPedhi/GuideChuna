...
public class CervicalRomPlaneGauge : MonoBehaviour
{
    ...
    /// <summary>지금 실측 외형인가. 판·채움이 꺼진 상태다.</summary>
    private bool realityLook;

    /// <summary>
    /// 실측 전환용. 판·채움을 끄고 눈금·지침·마커·화살표만 남긴다.
    /// ★면수직 오프셋(밀어내기)과 시야 자동 뒤집기는 <b>그대로 둔다</b> —
    ///   09-01에 이 둘을 껐다가 되돌렸다. 뒤집힘의 원인이 아니었고, 밀어내기는
    ///   실측에서도 쓸모가 있다(사용자: "밀어내는 건 오히려 좋으니까 냅둬").
    /// </summary>
    ...
    private Vector3 NormalOffsetOf(CervicalRomDriver.Direction d)
    {
        ...
        // ★2026-09-01 오진 정정 — 한때 실측에서 이 오프셋을 껐다가 되돌렸다.
        //   "각도기가 뒤집힌다"의 원인으로 지목했는데 틀렸다. 좌우 래치는 <b>면이 바뀔 때만</b>
        //   다시 정하므로(굴곡→신전은 같은 시상면이라 안 바뀐다) 여기서 뒤집힐 수가 없다.
        //   진범은 실측 축을 방향마다 다시 세우던 것이었다.
        //   사용자: "밀어내는 건 오히려 좋으니까 냅둬."
